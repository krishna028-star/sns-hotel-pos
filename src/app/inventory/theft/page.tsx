...
function InventoryTheft() {
  ...
  const submitReport = () => {
    ...
    setReports(prev => [
      { id: Date.now(), ingredient: form.ingredient, qty: parseFloat(form.qty), unit: form.unit, loss: parseFloat(form.loss) || 0, date: new Date().toISOString().split('T')[0], status: 'submitted', hotel: 'SNS Beach Resort', notes: form.description },
      ...prev
    ]);
    setShowModal(false);
  ...
  return (
    <DashboardLayout title="Theft Reports">
      ...
      <div className="metrics-grid" style={{ gridTemplateColumns: 'repeat(3,1fr)', marginBottom: 24 }}>
        {[
          { label: 'Submitted', value: reports.filter(r => r.status === 'submitted').length, color: '#FF8A34', bg: '#fff3e8' },
          { label: 'Verified', value: reports.filter(r => r.status === 'verified').length, color: '#FF3B30', bg: '#fff0ef' },
          { label: 'Total Loss', value: `₹${reports.reduce((a, r) => a + r.loss, 0).toLocaleString()}`, color: '#9B59B6', bg: '#f5f0ff' },
        ].map(m => (
